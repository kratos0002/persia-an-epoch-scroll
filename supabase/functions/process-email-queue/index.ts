...
Deno.serve(async (req) => {
  ...
  for (const queue of ['auth_emails', 'transactional_emails']) {
    ...
    for (let i = 0; i < messages.length; i++) {
      ...
      try {
        // Auth emails have run_id from webhook → use sendLovableEmail
        // Transactional emails have no run_id → use raw HTTP fetch
        if (payload.run_id) {
          ...
          await sendViaHttp(apiKey, Deno.env.get('LOVABLE_SEND_URL'), payload)
        }
