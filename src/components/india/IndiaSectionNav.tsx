...
interface IndiaSectionNavProps {
  activeSection: string;
}

export const IndiaSectionNav: React.FC<IndiaSectionNavProps> = ({ activeSection }) => {
    ...
    <>
      ...
      >
        <motion.div className="flex flex-col gap-1 items-center" animate={isOpen ? 'open' : 'closed'}>
          <motion.span className="block w-4 h-px" style={{ background: 'hsl(40, 25%, 70%)' }} variants={{ open: { rotate: 45, y: 2.5 }, closed: { rotate: 0, y: 0 } }} />
          <motion.span className="block w-4 h-px" style={{ background: 'hsl(40, 25%, 70%)' }} variants={{ open: { opacity: 0 }, closed: { opacity: 1 } }} />
          <motion.span className="block w-4 h-px" style={{ background: 'hsl(40, 25%, 70%)' }} variants={{ open: { rotate: -45, y: -2.5 }, closed: { rotate: 0, y: 0 } }} />
        </motion.div>
      </button>
          ...
          <>
            ...
            >
              <div className="p-6 pt-20">
                <p className="text-xs tracking-[0.3em] uppercase mb-6 font-body" style={{ color: 'hsl(40, 30%, 45%)' }}>Table of Contents</p>
                <div className="space-y-1">
                  {INDIA_SECTIONS.map((section) => {
                    ...
                    return (
                      <button
                        ...
                        className={cn(
                          ...
                          isActive ? 'bg-muted/30 text-foreground' : 'text-muted-foreground hover:text-foreground hover:bg-muted/20'
                        )}
                      >
                        <span className={cn(
                          'w-2 h-2 rounded-full shrink-0 transition-colors',
                          isActive ? 'bg-primary' : 'bg-muted-foreground/30 group-hover:bg-muted-foreground/60'
                        )} style={isActive ? { background: 'hsl(40, 60%, 55%)' } : {}} />
                        ...
                        {isActive && (
                          <span className="text-[9px] font-body px-2 py-0.5 rounded-full" style={{ background: 'hsl(40, 60%, 55%)', color: 'hsl(220, 20%, 10%)' }}>
                            {section.stateCount}
                          </span>
                        )}
                      </button>
                ...
                <div className="mt-8 pt-6 border-t border-border/20">
                  <p className="text-xs font-body" style={{ color: 'hsl(40, 15%, 40%)' }}>An Epoch Lives Visual Essay</p>
                </div>
              </div>
            </motion.nav>
          </>
