...
interface BuddhismSectionNavProps {
  activeSection: string;
}

export const BuddhismSectionNav = ({ activeSection }: BuddhismSectionNavProps) => {
    ...
    <>
      ...
      >
        <motion.div className="flex flex-col gap-1 items-center" animate={isOpen ? 'open' : 'closed'}>
          <motion.span className="block w-4 h-px" style={{ background: SAFFRON }} variants={{ open: { rotate: 45, y: 2.5 }, closed: { rotate: 0, y: 0 } }} />
          <motion.span className="block w-4 h-px" style={{ background: SAFFRON }} variants={{ open: { opacity: 0 }, closed: { opacity: 1 } }} />
          <motion.span className="block w-4 h-px" style={{ background: SAFFRON }} variants={{ open: { rotate: -45, y: -2.5 }, closed: { rotate: 0, y: 0 } }} />
        </motion.div>
      </button>
          ...
          <>
            ...
            >
              <div className="p-6 pt-20">
                <p className="text-xs tracking-[0.3em] uppercase mb-6 font-body" style={{ color: 'hsl(30, 65%, 45%, 0.5)' }}>Table of Contents</p>
                <div className="space-y-1">
                  {BUDDHISM_SECTIONS.map(section => {
                    ...
                    return (
                      <button
                        ...
                        style={isActive ? { background: 'hsl(30, 65%, 45%, 0.1)' } : undefined}
                      >
                        <span className={cn(
                          "w-2 h-2 rounded-full shrink-0 transition-colors",
                          isActive ? "" : "bg-muted-foreground/30 group-hover:bg-muted-foreground/60"
                        )} style={isActive ? { background: SAFFRON } : undefined} />
